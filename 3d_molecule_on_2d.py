...
class PluginSettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.sld_strength.valueChanged.connect(
            lambda v: self.lbl_strength.setText(f"{v}%")
        )
        ...
        self.chk_force_direct.setToolTip(
            "Uses 2D coordinates directly. Only available if 'Embed without H' is active."
        )
...
def on_cleanup_triggered(*args, allow_trigger=True, **kwargs):
        ...
        if needs_3d_refresh and allow_trigger:
            ...
            if _embed_without_h or _force_direct_mode:
                context.show_status_message(
                    f"Smart 3D: {status_msg} Local Embedding starting (threaded)..."
                )
                start_local_embedding(mw, _embed_without_h, _force_direct_mode)
            else:
                context.show_status_message(
                    f"Smart 3D: {status_msg} Triggering Main Conversion..."
                )
    ...
    if (
        ...
        and hasattr(_rotate_tool_handler, "rotate_act")
        and _rotate_tool_handler.rotate_act
    ):
...
def initialize(context):
    ...
    context.add_menu_action(
        "Settings/3D Molecule on 2D...", lambda: open_settings_dialog(mw, context)
    )
    ...
    if _enabled:
        context.add_toolbar_action(
            on_cleanup_triggered, "Clean Up 3D", tooltip="Sync 2D layout to 3D"
        )
        context.add_toolbar_action(
            lambda: None, "Rotate 3D", tooltip="Rotate molecule in 3D"
        )
        context.add_toolbar_action(
            show_settings_dialog, "3D on 2D Settings...", tooltip="Fine-tune visuals"
        )
...
def enable_plugin(mw, context):
    ...
    if pm and hasattr(pm, "toolbar_actions"):
        already = any(
            a.get("text", None) in _OWN_ACTION_TEXTS for a in pm.toolbar_actions
        )
        if not already:
            context.add_toolbar_action(
                on_cleanup_triggered, "Clean Up 3D", tooltip="Sync 2D layout to 3D"
...
def configure_actions():
    ...
    found_actions = [
        a
        for a in tb.actions()
        if a.text() in ["Clean Up 3D", "Rotate 3D", "3D on 2D Settings..."]
    ]
...
def on_rotate_toggled(checked):
    ...
    if checked:
        ...
        if (
            hasattr(mw.init_manager, "mode_actions")
            and "select" in mw.init_manager.mode_actions
        ):
...
def load_state(data):
    global _depth_cue_strength, _3d_scale, _embed_without_h
    mw = _mw
    if not mw or not data:
        return
...
def toggle_monkey_patches(active, mw=None):
    ...
    if active:
        ...
        if not hasattr(MainWindowCompute, "_original_on_calculation_finished"):
            MainWindowCompute._original_on_calculation_finished = (
                MainWindowCompute.on_calculation_finished
            )
    ...
    if _show_depth_cues and getattr(self, "z_3d", None) is not None and self.scene():
        ...
        if _show_depth_cues and z_range > 1e-4:
            ...
            if white_factor > 0.05:
                ...
                sym = self.symbol
                orig_color = atom_item.CPK_COLORS.get(
                    sym, atom_item.CPK_COLORS["DEFAULT"]
                )
                ...
                try:
                    ...
                    if had_key:
                        atom_item.CPK_COLORS[sym] = orig_color
                    else:
                        atom_item.CPK_COLORS.pop(sym, None)
                return
...
def patched_bond_paint(self, painter, option, widget):
    ...
    if (
        _show_depth_cues
        and hasattr(self.atom1, "z_3d")
        and hasattr(self.atom2, "z_3d")
        and self.scene()
    ):
    ...
    def run(self):
        try:
                ...
                for bidx, stype in stereo_info.items():
                    if bidx < m.GetNumBonds():
                        ...
                        if b.GetBondType() == Chem.BondType.DOUBLE:
                            ...
                            bn = [
                                n.GetIdx()
                                for n in begin.GetNeighbors()
                                if n.GetIdx() != end.GetIdx()
                            ]
                            en = [
                                n.GetIdx()
                                for n in end.GetNeighbors()
                                if n.GetIdx() != begin.GetIdx()
                            ]
                            if bn and en:
            ...
            if res == -1 or mol.GetNumConformers() == 0:
                self.status.emit(
                    "Embedding failed. Using direct coordinate fallback..."
                )
...
def start_local_embedding(mw, embed_without_h=False, force_direct_mode=False):
    ...
    thread = QThread()
    worker = LocalCalculationWorker(
        mol_block, embed_without_h, force_direct_mode, atom_ids
    )
...
def on_embedding_finished(mw, mol):
    ...
    if (
        ...
        try:
            ...
            QTimer.singleShot(
                100, lambda: _rotate_tool_handler.rotate_act.setChecked(True)
            )
        except Exception as _e:
...
def sync_to_3d_layout(mw, mol):
    ...
    if selected_items:
        for i, mol_atoms in enumerate(molecules):
            if any(a.isSelected() for a in mol_atoms):
                ...
                continue
            for bond in all_bonds:
                if bond.isSelected() and (
                    bond.atom1 in mol_atoms or bond.atom2 in mol_atoms
                ):
                    target_mol_indices.append(i)
                    break
    ...
    for i, mol_atoms in enumerate(molecules):
        ...
        scene_atom_ids = {
            getattr(a, "atom_id", None)
            for a in mol_atoms
            if getattr(a, "atom_id", None) is not None
        }
        best_frag_idx = None
        best_score = -1
...
class RotateToolHandler(QObject):
    ...
    def set_active(self, state):
        ...
        if state:
            ...
            if hasattr(self.mw.ui_manager, "activate_select_mode"):
                self.mw.ui_manager.activate_select_mode()
    ...
    def ensure_z_coords(self, force=False):
        ...
        all_atoms = [
            i
            for i in self.mw.scene.items()
            if type(i).__name__ == "AtomItem" and not sip_isdeleted_safe(i)
        ]
    ...
    def rotate_molecule(self, dx, dy):
        ...
        Rx = np.array(
            [[1, 0, 0], [0, np.cos(dy), -np.sin(dy)], [0, np.sin(dy), np.cos(dy)]]
        )
        Ry = np.array(
            [[np.cos(dx), 0, np.sin(dx)], [0, 1, 0], [-np.sin(dx), 0, np.cos(dx)]]
        )
        R = Rx @ Ry

        for mol_atoms in [self.target_atoms]:
            points = np.array(
                [[a.pos().x(), a.pos().y(), getattr(a, "z_3d", 0.0)] for a in mol_atoms]
            )
            ...
            for i, atom in enumerate(mol_atoms):
                ...
                if aid is not None and hasattr(
                    self.mw.state_manager.data, "set_atom_pos"
                ):
